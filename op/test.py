...
def infer_instance(model, instance, n_ants, k_sparse, t_aco_diff):
    ...
    aco = ACO(distances, prizes, MAX_LEN[len(prizes)], n_ants, heuristic=heu_mat, device=DEVICE, k_sparse=k_sparse)
...
def test(dataset, model, n_ants, k_sparse, t_aco):
    ...
    for instance in tqdm(dataset, dynamic_ncols=True):
        results, diversities = infer_instance(model, instance, n_ants, k_sparse, t_aco_diff)
...
if __name__ == "__main__":
    ...
    test_list = load_test_dataset(args.nodes, args.k_sparse, DEVICE)
    ...
    avg_cost, avg_diversity, duration = test(test_list, net, args.n_ants, args.k_sparse, t_aco)
    ...
    dirname = os.path.dirname(args.path) if args.path is not None else f'../pretrained/op/{args.nodes}/no_model'
    os.makedirs(dirname, exist_ok=True)

    result_filename = f"test_result_ckpt{filename}-op{args.nodes}-ninst{args.size}-nants{args.n_ants}-niter{args.n_iter}-seed{args.seed}"
